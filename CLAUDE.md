...
**Agentic Modern Web Standard**: Baseline target is Baseline Widely Available. Before app, extension, docs, UI, CSS, accessibility, browser proof, or web-design changes, search and retrieve current Chrome Modern Web Guidance, then apply `DESIGN.md`, the relevant surface dialect, and `packages/shared/src/styles/tokens.css`. Prefer semantic HTML, native controls, platform CSS, and browser primitives before custom JavaScript. Keep landmarks, headings, buttons, forms, labels, accessible names, visible focus, keyboard paths, touch targets, loading/error/empty states, and reduced-motion handling clear in the DOM and accessibility tree. Treat `bun run agentic:browser-proof` and narrower Playwright/visual suites as CI/clean-room evidence only unless they attach to authenticated Brave; they must not be reported as local authenticated verification. Local agentic browser QA must use the authenticated Brave QA profile. Codex: use the Codex browser-extension path and claim the already-open Brave tab/window. Claude Code: use the Claude Code Chrome/Chromium extension path (`claude --chrome` or `/chrome`) and select the authenticated Brave profile/tab when it is installed, connected, and able to control the already-open Brave window. Do not fall back merely because the extension is branded Chrome. If the Brave extension path is unavailable or not connected, use Claude computer-use/visible desktop control of the already-open Brave window; if neither can reach authenticated Brave, report QA as blocked. Do not use isolated Browser, Playwright, or DevTools MCP profiles for local QA. WebMCP is strategy-only in v1; do not ship runtime WebMCP tools unless explicitly requested, follow `docs/agentic-webmcp-strategy.md` for runtime prep, and never expose private local data, hidden extension state, session permissions, signing/publish actions, sync internals, destructive operations, or background-only actions.
