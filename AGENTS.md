...
- Baseline target: Baseline Widely Available. Before app, extension, docs, UI, CSS, accessibility, browser proof, or web-design changes, run repo-installed Modern Web Guidance with `bun run agentic:guidance`, then apply `DESIGN.md`, the relevant surface dialect, and `packages/shared/src/styles/tokens.css`.
