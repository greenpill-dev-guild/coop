...
const coopSpacePresets = listCoopSpacePresets();
...
function formatSharedWalletMode(mode: IntegrationMode) {
...
export function SidepanelApp() {
  ...
  // Initialize the inference bridge once and subscribe to status changes.
  useEffect(() => {
  ...
  // Sync the bridge opt-in state when the dashboard reports the persisted preference.
  useEffect(() => {
  ...
  async function handleExecuteWithGrant(
    ...
    await sendRuntimeMessage({
      ...
      payload: { grantId, replayId, actionClass, coopId: activeCoopId ?? '', actionPayload },
    });
    await loadDashboard();
  }
  ...
  function renderDraftCard(draft: ReviewDraft, context: 'roost' | 'meeting') {
    ...
    return (
      <article className="draft-card stack" key={draft.id}>
        ...
        {refineResults[draft.id] ? (
          <div
            className="panel-card"
            style={{ background: 'var(--surface-alt, #f0f0f0)', padding: '0.5rem' }}
          >
            <strong>Polish suggestion</strong>
            <span className="badge">{refineResults[draft.id].provider}</span>
            {refineResults[draft.id].refinedTitle ? (
              <div className="field-grid">
                <span className="helper-text">Title</span>
                <span>{refineResults[draft.id].refinedTitle}</span>
              </div>
            ) : null}
            {refineResults[draft.id].refinedSummary ? (
              <div className="field-grid">
                <span className="helper-text">Summary</span>
                <span>{refineResults[draft.id].refinedSummary}</span>
              </div>
            ) : null}
            {refineResults[draft.id].suggestedTags ? (
              <div className="field-grid">
                <span className="helper-text">Tags</span>
                <span>{refineResults[draft.id].suggestedTags?.join(', ')}</span>
              </div>
            ) : null}
            <div className="action-row">
              <button
                className="primary-button"
                onClick={() => applyRefineResult(draft)}
                type="button"
              >
                Apply
              </button>
              <button
                ...
                onClick={() => dismissRefineResult(draft.id)}
                type="button"
              >
                Dismiss
              </button>
        ...
        <div className="action-row">
          {inferenceState?.capability.status !== 'disabled' ? (
            <button
              className="secondary-button"
              disabled={refiningDrafts.has(draft.id)}
              onClick={() => void refineDraft(draft, 'summary-compression')}
              type="button"
            >
              {refiningDrafts.has(draft.id) ? 'Polishing...' : 'Polish locally'}
            </button>
          ) : null}
        ...
        {panelTab === 'Loose Chickens' && (
          <section className="panel-card">
            ...
            <div className="action-row">
              ...
              <button className="secondary-button" onClick={runActiveTabCapture} type="button">
                Catch this tab
              </button>
              <button
                className="secondary-button"
                onClick={captureVisibleScreenshotAction}
                type="button"
              >
                Snap this page
              </button>
              ...
              <form className="form-grid" onSubmit={createCoopAction}>
                <div className="detail-grid">
                  <div className="field-grid">
                    ...
                    >
                      {coopSpacePresets.map((preset) => (
                        <option key={preset.id} value={preset.id}>
                          {preset.label}
                        </option>
                      ))}
                    </select>
                    <span className="helper-text">{selectedSpacePreset.description}</span>
                  </div>
                  <div className="field-grid">
                    ...
                    <input
                      ...
                      placeholder={`${selectedSpacePreset.label} name`}
                      required
                  ...
                  <div className="field-grid">
                    ...
                    <input
                      ...
                      placeholder={selectedSpacePreset.purposePlaceholder}
                      required
                ...
                <div className="field-grid">
                  ...
                  <textarea
                    ...
                    placeholder={selectedSpacePreset.summaryPlaceholder}
                    required
                ...
                <div className="field-grid">
                  ...
                  <textarea
                    ...
                    placeholder={selectedSpacePreset.seedContributionPlaceholder}
                    required
                    ...
                    <div className="field-grid">
                      ...
                      <label className="helper-text" htmlFor="green-goods-garden">
                        <input
                          ...
                          type="checkbox"
                          checked={createForm.createGreenGoodsGarden}
                          onChange={(event) =>
                            setCreateForm((current) => ({
                              ...current,
                      ...
                      <span className="helper-text">
                        {selectedSpacePreset.greenGoodsRecommended
                    ...
                    <div className="lens-grid">
                      {[
                        [
                          ...
                          'capitalImprove',
                          'Money & resources',
                          selectedSpacePreset.lensHints.capital,
                        ],
                        [
                          'impactCurrent',
                          'impactPain',
                          'impactImprove',
                          'Impact & outcomes',
                          selectedSpacePreset.lensHints.impact,
                        ],
                        [
                          'governanceCurrent',
                          'governancePain',
                          'governanceImprove',
                          'Decisions & teamwork',
                          selectedSpacePreset.lensHints.governance,
                        ],
                        [
                          'knowledgeCurrent',
                          'knowledgePain',
                          'knowledgeImprove',
                          'Knowledge & tools',
                          selectedSpacePreset.lensHints.knowledge,
                        ],
            ...
            <article className="panel-card">
              ...
              <div className="detail-grid archive-detail-grid">
                ...
                <div>
                  ...
                  <p className="helper-text">{formatSharedWalletMode(runtimeConfig.archiveMode)}</p>
                </div>
                <div>
              ...
              <div className="action-row">
                <a
                  className="secondary-button"
                  href={runtimeConfig.receiverAppUrl}
                  rel="noreferrer"
                  target="_blank"
                >
                  Open Pocket Coop
