...
export function OperatorConsole(props: OperatorConsoleProps) {
  ...
  return (
    <>
      ...
      <article className="panel-card">
        <h2>Trusted Nest Controls</h2>
        <p className="helper-text">
          Trusted mode makes this browser the steady helper for live saves, deeper proof checks, and
          shared-wallet steps.
        </p>
      ...
      <article className="panel-card">
        ...
        <p className="helper-text">Choose which actions always need a human yes before they run.</p>
      ...
      <article className="panel-card">
        ...
        {props.sessionCapabilities.map((capability) => (
          <article className="operator-log-entry" key={capability.id}>
            <div className="badge-row">
              <span className="badge">{formatSessionCapabilityStatusLabel(capability.status)}</span>
        ...
        {props.sessionCapabilityLog.slice(0, 20).map((entry) => (
          <article className="operator-log-entry" key={entry.id}>
            <div className="badge-row">
              ...
              {entry.reason ? (
                <span className="badge">{formatSessionCapabilityFailureReason(entry.reason)}</span>
              ) : null}
            </div>
            <strong>{entry.detail}</strong>
            <div className="helper-text">{new Date(entry.createdAt).toLocaleString()}</div>
