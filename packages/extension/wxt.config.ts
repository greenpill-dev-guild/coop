...
function applySharedChunking(viteConfig: { build?: { rollupOptions?: { output?: unknown } } }) {
  ...
  // Only split third-party libs that are already on-demand and have no
  // cross-references back into `@coop/shared`. Splitting shared modules
  // (erc8004, fvm, stealth, archive, policy, greengoods) into dedicated
  // chunks broke the popup with a circular-import TDZ — the shared modules
  // reference each other through the `@coop/shared` barrel, and slicing
  // them into separate ES module chunks turned hoist-safe internal refs
  // into cross-chunk evaluation cycles. Let Rollup's default grouping
  // handle shared-module placement instead.
